...
describe('pool math', () => {
  it('should compute collateral balance from blockchain data', () => {
    ...
    // reserve: YFI
    const scaledMTokenBalance = '161316503206059870';
    ...
    const expectedMTokenBalance = '161594727054623229';
    const underlyingBalance = getLinearBalance({
      balance: scaledMTokenBalance,
      index: liquidityIndex,
      rate: currentLiquidityRate,
      lastUpdateTimestamp,
      currentTimestamp,
    }).toString();
    expect(underlyingBalance).toEqual(expectedMTokenBalance);
  });
